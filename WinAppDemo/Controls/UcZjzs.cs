...
namespace WinAppDemo.Controls
{
    public partial class UcZjzs : UserControl
    {
        ...
        private void dataGridView4_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //突出显示搜索到的词
            int SearchStart = 0, tempValue = 0;
            while (SearchStart < richTextBoxEx1.Text.Length)
            {
                //Find()函数有问题，有时会从尾巴调回头部搜索，陷入死循环 导致程序卡死，加一个临时变量判断一下
                tempValue = richTextBoxEx1.Find(keyword, SearchStart, RichTextBoxFinds.None);
                if (tempValue == -1 || tempValue < SearchStart) break;
                SearchStart = tempValue;
                richTextBoxEx1.SelectionStart = SearchStart;
                richTextBoxEx1.SelectionLength = keyword.Length;
                richTextBoxEx1.SelectionColor = Color.Purple;
                richTextBoxEx1.SelectionFont = new Font(richTextBoxEx1.Font.FontFamily, 12, FontStyle.Bold);
